...
function compile_inner(expr::App, env::Env, path_condition::BDD, state::LazyKCState)
    ...
    return bind_monad(fs, path_condition, state) do f, path_condition, state
        new_env = copy(f.env)
        x = thunked_argument
        pushfirst!(new_env, x)
...
function compile_inner(expr::CaseOf, env::Env, path_condition::BDD, state::LazyKCState)
    scrutinee_worlds = traced_compile_inner(expr.scrutinee, env, path_condition, state, 0)
    ...
    bind_monad(scrutinee_worlds, path_condition, state) do scrutinee, path_condition, state
...
function compile_prim(op::FlipOp, args, env::Env, path_condition::BDD, state::LazyKCState)
    ...
    bind_monad(ps, path_condition, state) do p, path_condition, state
        p = p.value
        if isapprox(p, 0.0)
...
function compile_prim(op::FlipOpDual, args, env::Env, path_condition::BDD, state::LazyKCState)
    ...
    bind_monad(metaparams, path_condition, state) do metaparam, path_condition, state
...
function compile_prim(op::ConstructorEqOp, args, env::Env, path_condition::BDD, state::LazyKCState)
    ...
    bind_monad(first_arg_results, path_condition, state) do arg1, path_condition, state
        second_arg_results = traced_compile_inner(args[2], env, path_condition, state, 1)
        bind_monad(second_arg_results, path_condition, state) do arg2, path_condition, state
...
function compile_prim(op::IntDistEqOp, args, env::Env, path_condition::BDD, state::LazyKCState)
    ...
    bind_monad(first_int_dist, path_condition, state) do first_int_dist, path_condition, state
        second_int_dist = traced_compile_inner(args[2], env, path_condition, state, 1)
        bind_monad(second_int_dist, path_condition, state) do second_int_dist, path_condition, state
