...
function io_constrain(expr, io, eval_state::EvalState)
    if eval_state.config.top_level_equality
        expr = io_equality_expr(expr, io.inputs, io.output)
        (spn, eval_state) = constrain(expr, [], true, eval_state)
    else
        (spn, eval_state) = constrain(expr, io.inputs, io.output, eval_state)
    end
    w = weight(eval_state.spn_hash, spn)
    return IOConstrainResult(w, eval_state.stats)
end
