...
function make_self_loop(body, env)
    ...
    # now put in the actual self reference if it was used (ie if it wasn't set to Nothing)
...
struct Thunk
    ...
    name::Symbol
    memoizing::Bool
    function Thunk(expr::PExpr, env::Vector{Any}, callstack, name, memoizing)
...
Base.hash(x::Value, h::UInt) = hash(x.spt, hash(x.constructor, hash(x.args, h)))
...
Value(val::Value) = val
Value(constructor::Symbol, args...) = Value(spt_of_constructor[constructor], constructor, Any[Value(arg) for arg in args])
Value(x::Float64) = x

function Value(c_and_args::Union{Vector, Tuple})
    length(c_and_args) == 1 && return Value(c_and_args[1])
    Value(c_and_args[1], c_and_args[2:end])
end
...
function from_value(x::Value)
    if x.constructor === :Unit
        return nothing
    elseif x.constructor === :True
        return true
    ...
    elseif x.constructor == :S
        (x.args[1] isa Thunk || x.args[1] isa LazyEnumeratorThunk || x.args[1] isa BDDThunk || x.args[1] isa BDDThunkUnion)  && return x
...
function make_list_from_julia_list(l::Vector)
    if isempty(l)
        return "(Nil)"
    else
...
make_list_from_julia_list(l::Bool) = l ? "(True)" : "(False)"
make_list_from_julia_list(l::Nothing) = "(Unit)"

# @define bool_eq = "(λ x y -> (case x of True => ))"