...
struct ConstSymbol <: PExpr
    name::Symbol
end
num_children(e::ConstSymbol) = 0
unifies(e1::ConstSymbol, e2::ConstSymbol) = e1.name == e2.name
...
function parse_expr_inner(tokens::Vector, defs, env)
    if length(tokens) == 0
    ...
    if token == "("
        ...
        elseif token == "if"
            ...
            # return If(cond, then_expr, else_expr), tokens[2:end]
            return CaseOf(cond, Dict(:True => then_expr, :False => else_expr), [:True, :False]), tokens[2:end]
        elseif token == "ylamlam"
            # Parse a Y lam lam
            tokens = tokens[2:end]
            body, tokens = parse_expr_inner(tokens, defs, env)
            tokens[1] != ")" && error("expected closing paren")
            return Ylamlam(body), tokens[2:end]
        elseif token == "Y"
            # parse a Y
            tokens = tokens[2:end]
        ...
        elseif token == "case"
            ...
            while tokens[1] != ")"
                ...
                if tokens[1] == "=>"
                    body, tokens = parse_expr_inner(tokens[2:end], defs, env)
                else
            ...
            return CaseOf(scrutinee, cases, constructors), tokens[2:end]
        elseif token == "let"
            ...
            tokens = tokens[2:end]  # Skip closing paren of bindings list
            body, tokens = parse_expr_inner(tokens, defs, env)
        ...
        elseif haskey(spt_of_constructor, Symbol(token))
            ...
            return Construct(spt, constructor, args), tokens[2:end]
        elseif haskey(primops, token) && !haskey(defs, Symbol(token))
            ...
            tokens[1] != ")" && error("expected closing paren")
            return PrimOp(op, args), tokens[2:end]
        else
            ...
            # @show length(args)
            @assert !isempty(args) "empty args for $f" # i know we should support this but in deadline rush i am not
            expr = f
            for arg in args
                expr = App(expr, arg)
            end
    ...
    elseif token == "??"
        return NULL, tokens[2:end]
    elseif token[1] == '?'
        name = Symbol(token[2:end])
        return AuxCFGSymbol(name), tokens[2:end]
    elseif token[1] == '~'
        type = parse_type(token[2:end])
        return RandomnessCFGSymbol(type), tokens[2:end]
    elseif token[1] == ':'
        name = Symbol(token[2:end])
        return ConstSymbol(name), tokens[2:end]
    elseif token ∈ env
        # Parse a var by name like "foo"
        idx = findfirst(x -> x == token, env) # shadowing
        return Var(idx, Symbol(token)), tokens[2:end]
    elseif token[1] == '#'
        if isdigit(token[2])
            # parse debruijn index
            idx = parse(Int, token[2:end])
            @assert idx > 0 "need one-index debruijn"
            return Var(idx), tokens[2:end]
        else
            # parse CFG symbol variable like "#int"
            type = parse_type(token[2:end])
            return VarCFGSymbol(type), tokens[2:end]
        end
    elseif '#' ∈ token
    ...
    elseif all(isdigit, token)
        val = parse(Int, token)
        return const_to_expr(val), tokens[2:end]
    elseif all(c -> isdigit(c) || c == '.' || c == '-' && length(token) > 1, token)
    ...
    elseif haskey(defs, Symbol(token))
        # return defs[Symbol(token)], tokens[2:end]
        # TODO: do gensyms need to be regenerated?
        return Defined(Symbol(token)), tokens[2:end]
    else
...
function parse_expr(s::String, defs)
    ...
    @assert isempty(rest) "when parsing $s encountered trailing tokens: $rest"
    return expr
end
...
var_is_free(e::ConstSymbol, var) = false
...
Base.show(io::IO, e::ConstSymbol) = print(io, ":", e.name)
...
Base.copy(e::ConstSymbol) = ConstSymbol(e.name)
...
Base.:(==)(a::ConstSymbol, b::ConstSymbol) = a.name == b.name
Base.:(==)(a::If, b::If) =
...
Base.hash(e::ConstSymbol, h::UInt) = hash(e.name, hash(:ConstSymbol, h))
...
function get_func_type(f::Defined, env)::PType
    res = get_def(f.name).type
    @assert res isa PType "function $f has no type annotation"
    res
end
