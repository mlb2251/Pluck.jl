export Grammar, propose, SMCProposalResult, ExpandLeafProposal, ExpandLeafMultistepProposal, Geometric, show_size_dist
...
function show_size_dist(g::Grammar, max_size)
    ...
    for prod in g.prods
        sym = prod.lhs
        println(rpad(sym, pad_sym), " ", collect(round3(size_dist(g, sym, k; cache = cache)) for k = 0:max_size))
        pad_rhs = maximum(length(string(prod.rhs_all[i])) for i in eachindex(prod.rhs_all))
        for i in eachindex(g.prod_of_sym[sym].rhs_all)
            println("  ", rpad(g.prod_of_sym[sym].rhs_all[i], pad_rhs), " ", collect(round3(size_dist(g, sym, i, k; cache = cache)) for k = 0:max_size))
        end
        println()
    end
end
...
function sum_to_k(pcfg::Grammar, symbol_paths::Vector{Tuple{Symbol, Path}}, remaining::Int; cache = SizeCache())::Float64
    ...
    sum(size_dist(pcfg, first_sym, first_size; cache = cache) * sum_to_k(pcfg, symbol_paths, remaining - first_size; cache = cache) for first_size = 0:remaining)
end
...
function Grammar(prods, sym_of_type::Vector{Pair{String, String}}, start_expr_of_type::Vector{Pair{String, String}}; size_dist = Geometric(0.5), start_sym_of_type::Vector{Pair{String, String}} = sym_of_type)
    ...
    for (lhs_str, rhs_options_vec) in prods
        ...
        @assert rhs_options_vec isa Vector "rhs_options should be a list not a $(typeof(rhs_options)) on the RHS of $lhs_str"
        ...
        total_weight_novar = sum(wt for (e, wt) in rhs_entries if !(e isa VarCFGSymbol))
    ...
    for prod in unrolled_prods
        ...
        @assert sum(leaves) == 0.0 || sum(prod.probs_leaf_yesvar) ≈ 1.0
        @assert sum(leaves) == 0.0 || sum(prod.probs_leaf_novar) ≈ 1.0
        @assert sum(nonterms) == 0.0 || sum(prod.probs_nonterm_yesvar) ≈ 1.0
        @assert sum(nonterms) == 0.0 || sum(prod.probs_nonterm_novar) ≈ 1.0
        @assert sum(random_novar_leaves) == 0.0 || sum(prod.probs_leaf_novar_random) ≈ 1.0
    end
...
GExpr(lhs, expr) = GExpr(lhs, nothing, 0, -Inf, 0.0, expr, GExpr[], nothing)
...
function show_detailed(e::GExpr, depth = 0)
    ...
    alloc_str = e.size_allocation_logprob == 0.0 ? "" : " <- $(round3(exp(e.size_allocation_logprob)))"
...
function modify_expression(
    ...
    log_proposal_ratio = log_backward_proposal - log_forward_proposal

    return e, log_proposal_ratio, selected_subexpression.expr.path
end
