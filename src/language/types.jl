...
function define_type!(type::Symbol, constructors::Dict{Symbol,Vector{Symbol}})
    @assert !haskey(constructors_of_type, type) "type $type already defined"
    constructors_of_type[type] = Symbol[]
    for (constructor, args) in constructors
        @assert !haskey(type_of_constructor, constructor) "constructor $constructor already defined for another type $(type_of_constructor[constructor])"
