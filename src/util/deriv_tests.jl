...
function deriv_tests()
    ...
    used_vars = max_native_int_used(parse_expr("@0"))
    @assert used_vars == 0

    used_vars = max_native_int_used(parse_expr("(let (a (flip @0)
                                b (not (flip @1)))
                                (and a b))"))
    @assert used_vars == 1

    used_vars = max_native_int_used(parse_expr("(flip @0)"))
    @assert used_vars == 0

    used_vars = max_native_int_used(parse_expr("(flip 0.2)"))
    @assert used_vars == -1
